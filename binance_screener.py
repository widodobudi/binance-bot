...
OPEN LONG -- 6 syarat pada candle HARIAN (1D):
...
def thread1_scan():
    ...
    if not candidates:
        log(f"[T1] {len(universe)} coin discan, tidak ada yg lolos 6 syarat.")
        last_processed_candle_ts = newest_ts
        return

    # pilih 1 (slot max 1) — pilih ATR% terkecil (paling stabil) sebagai tie-break
    candidates.sort(key=lambda x: x[2])
    sym, price, atrp = candidates[0]
    log(f"[T1] SINYAL: {sym} close={price:.6g} atr%={atrp:.2f} (dari {len(candidates)} kandidat)")

    if send_open_long(sym):
        add_to_active_deals(sym, {
            'entry_price': price, 'peak': price,
            'atr_pct': atrp, 'opened_candle_ts': int(newest_ts),
...
if __name__ == '__main__':
    ...
    log(f"  Entry 6 syarat   : ST-up, >EMA20, EMA20>EMA50, breakout{BREAKOUT_LOOKBACK}, vol>={VOLUME_MULT}xMA, RSI<{RSI_MAX}")
    ...
    send_telegram(
        ...
        f"Entry: ST-up + >EMA20 + EMA20>EMA50 + breakout{BREAKOUT_LOOKBACK} + vol>={VOLUME_MULT}xMA + RSI<{RSI_MAX}\n"
