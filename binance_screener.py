...
<div class="container">
  <div class="card">
    ...
    <table>
      ...
      <tr>
        ...
        <td>{{ "%.4g"|format(d.get("entry_price",0)) }}</td>
    ...
    <div class="card-body">
      ...
      <div style="margin-bottom:12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        ...
        <select id="pair-select" onchange="onPairSelect(this.value)" style="background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:4px;padding:5px 10px;font-size:12px;font-family:var(--font);cursor:pointer;min-width:160px">
          ...
          <option value="{{ item.sym }}" data-vol="{{ item.vol_ratio if item.vol_ratio is not none else '' }}">{{ item.sym.replace("USDT","/USDT") }} ({{ item.n_pass }}/{{ item.total }})</option>
          {% endfor %}
        </select>
  ...
  <!-- Filter vol max untuk kandidat brkX2-12h -->
  <div style="margin-bottom:8px;display:flex;align-items:center;gap:10px;font-size:11px">
    <label style="color:var(--muted)">Filter kandidat brkX2-12h: sembunyikan vol &gt;</label>
    <input type="number" id="vol-max-filter" value="5" min="1" max="99" step="0.5"
      style="width:60px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:4px;padding:3px 6px;font-size:11px;font-family:var(--font)"
      onchange="applyVolFilter()" oninput="applyVolFilter()">
    <span style="color:var(--muted)">xMA &nbsp;(kosongkan = tampilkan semua)</span>
  </div>
  ...
  <div class="card">
    ...
    <table>
      ...
      <tbody>
      {% for item in items %}
      <tr data-vol="{{ item.vol_ratio if item.vol_ratio is not none else '' }}" data-strat="{{ strategi }}">
...
<script src="/dash.js?v=1785518128"></script>
</body>
</html>
'''
