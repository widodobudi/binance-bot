...
─────────────────────────────────────────────────────────
  ...
  └─ WINDOW 3  menit 170–209 (elapsed ~70–87%)
            ...
            G5. LAPIS 2 — BTC filter:
                BTC ≥ EMA20×0.98
                DAN c-3 doji HA (body <25% range)
                DAN c-2 HA bullish (sudah tutup)
                DAN c-1 HA bullish (sudah tutup)
                DAN c-0 running HA bullish + elapsed ≥30%
            ...
            if active_count >= COMMAS_MAX_ACTIVE_DEALS:
                ...
                if should_confirm_alert(best_symbol):
                    ...
                    if btc_df is not None and len(btc_df) >= 21:
                        ...
                        #   DAN c-3 doji HA (body < 25% dari range)
                        #   DAN c-2 HA bullish (sudah tutup)
                        #   DAN c-1 HA bullish (sudah tutup)
                        #   DAN c-0 running HA bullish dengan elapsed >= 30%
                        def calc_ha(df, idx):
                        ...
                        if btc_ema20_v is not None and len(btc_df) >= 5:
                            ...
                            if not price_ok or not c3_doji or not c2_ha_bull or not c1_ha_bull or not c0_ok:
                                ...
                                btc_bypass_reason = (
                                    ...
                                    f"c-3 doji={c3_str} c-2 HA bull={c2_str} c-1 HA bull={c1_str} c-0 HA bull+30%={c0_str}"
                                )
                    ...
                    if not btc_bypass and btc_df is not None and len(btc_df) >= 5:
                        ...
                        log(f"   [T1] BTC Lapis2 OK: {float(btc_df.iloc[-1]['close']):,.0f} ({pct_vs_ema2:+.1f}% vs EMA20) | c-3 doji=✅ c-2 HA bull=✅ c-1 HA bull=✅ c-0 HA bull+30%=✅")
...
if __name__ == '__main__':
    ...
    log("    └─ Gerbang: Lapis2(BTC≥EMA20×0.98+c3 doji HA+c2/c1 HA bull+c0 HA bull≥30%) → Elapsed≤75%")
    ...
    send_telegram(
        ...
        "   <b>Lapis 2</b> BTC: harga≥EMA20×0.98 + c-3 doji HA + c-2/c-1 HA bullish + c-0 HA bullish (≥30% elapsed)\n"
