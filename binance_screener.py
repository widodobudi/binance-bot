...
def thread2_monitor():
    ...
    for sym in syms:
        ...
        # batas hold = MAX_HOLD_DAYS candle (di 12h = 5*12jam=2.5 hari; di 1d = 5 hari)
        opened_ts = d.get('opened_candle_ts',0)/1000.0
        if opened_ts>0 and (time.time()-opened_ts) >= MAX_HOLD_DAYS*SECONDS_PER_CANDLE:
            do_close=True; reason=f"batas {MAX_HOLD_DAYS} candle tercapai"
