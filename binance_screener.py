...
─────────────────────────────────────────────────────────
  ...
  FASE 1    Kumpulkan Kandidat Pool (sepanjang candle)
            Scan 400+ pair, cek 7 syarat:
            1. Chg 24H antara −8% s.d. −3% (terkoreksi ≥3%)
            2. Volume 24H ≥ $5.000.000
            3. Elapsed ≥ 10%
            4. RSI naik dari candle sebelumnya
            5. Stoch K > D
            6. Volume buy naik dari candle sebelumnya
            7. Harga dalam zona 0% s.d. −5% di bawah EMA20 4H
...
# POOL — Koreksi + Momentum Berbalik (v3):
#   1. Chg 24H -8% hingga -3% (terkoreksi minimal 3%)
#   2. Volume 24H >= $5M
#   3. Elapsed >= 10%
#   4. RSI sekarang > RSI sebelumnya
#   5. Stoch K > D
#   6. Volume buy (tbbav) candle ini > candle sebelumnya
#   7. Harga di zona 0% s.d. -5% di bawah EMA20 4H
#   (Supertrend TIDAK lagi jadi syarat — st_line tetap dihitung untuk DistST/EstCandle)
#
# NOTIF POOL:
...
# ─────────────────────────────────────────────────────────────
CHG_24H_MIN              = -8.0   # v3: Chg 24H >= -8% (tidak terlalu crash)
CHG_24H_MAX              = -3.0   # v3: Chg 24H <= -3% (harus benar-benar terkoreksi ≥3%)
...
def check_screener(df, chg_24h, vol_usd=0):
    """
    Kondisi teknikal v3 — Koreksi + Momentum Berbalik (7 syarat).
    Syarat:
    1. Chg 24H -8% hingga -3%    (difilter di thread1_scan)
    2. Volume 24H >= $5M         (difilter di thread1_scan)
    3. Elapsed >= 10%
    4. RSI sekarang > RSI sebelumnya
    5. Stoch K > D
    6. Volume buy (tbbav) candle ini > candle sebelumnya
    7. Harga dalam zona 0% s.d. -5% di bawah EMA20 (4H) — tidak boleh di atas maupun lebih dari 5% di bawah EMA20
    (Supertrend bukan lagi syarat; st_line tetap dihitung untuk DistST & EstCandle di Fase 2)
    """
    ...
    # Supertrend: TIDAK lagi jadi syarat pool (gate downtrend dihapus v3).
    # Tetap dihitung karena st_line dipakai untuk DistST & EstCandle (gerbang open long Fase 2).
    try:
    ...
    # Syarat 8: Harga dalam zona 0% s.d. -5% di bawah EMA20 (4H)
    # - Harga >= EMA20         → masih premium, belum cukup koreksi → skip
    # - Harga < EMA20 × 0.95  → sudah terlalu dalam di bawah EMA20  → skip
    # Zona valid: EMA20 × 0.95 ≤ harga < EMA20  (v3: ganti EMA50 → EMA20)
    EMA20_MAX_DIST_PCT = -5.0   # % maksimal di bawah EMA20
    ema20_p     = ta.ema(close, length=20)
    ema20_pval  = ema20_p.iloc[-1] if ema20_p is not None else None
    if ema20_pval is None or pd.isna(ema20_pval):
        return False, {}
    price_now   = float(last['close'])
    ema20_pval  = float(ema20_pval)
    ema20_lower = ema20_pval * (1 + EMA20_MAX_DIST_PCT / 100)   # EMA20 × 0.95
    if price_now >= ema20_pval:     # di atas atau sama dengan EMA20 → skip
        return False, {}
    if price_now < ema20_lower:     # lebih dari 5% di bawah EMA20 → skip
        return False, {}
...
if __name__ == '__main__':
    ...
    log("  Lapis1 (BTC candle 4H berjalan ≤−3% dari open candle → scan batal) → Fase1 (pool 7 syarat)")
    ...
    send_telegram(
        ...
        "2️⃣ <b>Fase 1</b> — Pool: 7 syarat (Chg −8%..−3%, Vol, Elapsed, RSI, Stoch, VolBuy, EMA20 zona −5%)\n"
