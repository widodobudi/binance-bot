...
def heartbeat_general_tick():
    ...
    send_telegram(
        ...
        f"brkX2-4h   : ST-up + MACD>0 + ATR>=2% + vol>={STRAT4H_VOLUME_MULT}xMA + HTF 12h candle bullish\n"
...
def thread_akum_scan():
    ...
    try:
        ...
        if full_ok:
            ...
            for r in full_ok:
                gate_str = "✓ Gating OK" if r.get('gating_ok') else "⚠ Gating FAIL"
                lines.append(
                    ...
                    f"  SL-A est: {_fmt_price(r.get('support',0)*0.995)} | SL-B est: {_fmt_price(r.get('resistance',0)*0.995)}\n"
...
if __name__ == '__main__':
    ...
    if STRAT4H_ENABLED:
        ...
        log(f"  Entry: ST+1 + MACD>0 + ATR>={STRAT4H_ATR_MIN_PCT}% + Vol>={STRAT4H_VOLUME_MULT}xMA + RSI<60 + HTF {STRAT4H_HTF_TF} 3x candle bullish berturutan")
